...
export default function Dashboard() {
  ...
  useEffect(() => {
    // 🔌 API: GET /pets and GET /schedule/today
    ...
    <main className="app-shell animate-fade-in">
      ...
      <section className="px-6 mt-7">
        ...
        <div className="grid grid-cols-2 gap-3">
          <QuickAction label="Log Meal"   Icon={Utensils} variant="health"   onClick={handleLogMeal} />
          <QuickAction label="Log Weight" Icon={Scale}    variant="info"     onClick={handleLogWeight} />
          <QuickAction label="Add Note"   Icon={FileText} variant="schedule" onClick={handleAddNote} />
          <QuickAction label="Add Photo"  Icon={Camera}   variant="pink"     onClick={() => fileInputRef.current?.click()} />
        </div>
        <input
          ref={fileInputRef}
          type="file"
